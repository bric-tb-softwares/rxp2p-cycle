...
def save_images(webpage, images, names, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.is_resume_wandb = opt.is_resume_wandb
        self.wandb_fold_id = opt.wandb_fold_id
        ...
        if self.use_wandb:
            if self.is_resume_wandb:
                self.wandb_run = wandb.init(id=self.wandb_fold_id, project='BicycleGAN', resume='allow',
                                            name=opt.name, config=opt) if not wandb.run else wandb.run
            else:
                self.wandb_run = wandb.init(id=self.wandb_fold_id, project='BicycleGAN', name=opt.name,
                                            config=opt) if not wandb.run else wandb.run
            self.wandb_run._label(repo='BicycleGAN')
