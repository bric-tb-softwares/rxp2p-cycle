...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        if self.use_wandb:
            ...
            #  self.wandb_run = wandb.init(id = id_wandb, project='CycleGAN-and-pix2pix', name=opt.name, resume = "allow", config=opt) if not wandb.run else wandb.run
            # else:
            #  self.wandb_run = wandb.init(id = id_wandb, project='CycleGAN-and-pix2pix', name=opt.name, config=opt) if not wandb.run else wandb.run
