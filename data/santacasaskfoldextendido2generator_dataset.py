...
class SantaCasaSkfoldExtendido2GeneratorDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        try:
            ...
            #f_iltbi = df_iltbi.sort_values('project_id')

        
        path_santa_casa_anonimizado_valid = '/home/brics/public/brics_data/SantaCasa/imageamento_anonimizado_valid/raw/splits.pkl'
        masks_available = pd.read_csv('/home/brics/public/brics_data/SantaCasa/imageamento_anonimizado_valid/raw/dicionario_masks_santacasa_anonimizado_valid.csv')[['project_id','binary_limiar']].dropna()
        ...
        self.test = df_iltbi.iloc[splits[opt.sort][2]]



        if opt.train_dataset:
            print('Generating ILTBI TRAIN data: ' + str(len(self.train['project_id'].tolist())) + ' imgs')
            self.AB_paths = [opt.dataroot + img_path +'.png' for img_path in self.train['project_id'].tolist() if img_path in masks_available['project_id'].str.replace('mask_|.png','').tolist()]
        else:
            if self.gen_val:
                print('Generating ILTBI VAL data: ' + str(len(self.val['project_id'].tolist())) + ' imgs')
                self.AB_paths = [opt.dataroot + img_path +'.png' for img_path in self.val['project_id'].tolist() if img_path in masks_available['project_id'].str.replace('mask_|.png','').tolist()]
            if self.gen_test:
                print('Generating ILTBI TEST data: ' + str(len(self.test['project_id'].tolist())) + ' imgs')
                self.AB_paths = [opt.dataroot + img_path +'.png' for img_path in self.test['project_id'].tolist() if img_path in masks_available['project_id'].str.replace('mask_|.png','').tolist()]
