...
class SantaCasaSkfold2generatorDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        #self.train_tb = training_data
        #self.val_tb = validation_data
        #self.test_tb = test_data

        self.train_tb = training_data.loc[df.all_together == 'seg']
        self.val_tb = validation_data.loc[df.all_together == 'seg']
        self.test_tb = test_data.loc[df.all_together == 'seg']

        if opt.train_dataset:
            print('Generating ILTBI TRAIN data: ' + str(len(self.train_tb['images'].tolist())) + ' imgs')
            self.AB_paths = [opt.dataroot + img_path for img_path in self.train_tb['images'].tolist()]
        else:
            if self.gen_val:
                print('Generating ILTBI VAL data: ' + str(len(self.val_tb['images'].tolist())) + ' imgs')
                self.AB_paths = [opt.dataroot + img_path for img_path in self.val_tb['images'].tolist()]
            if self.gen_test:
                print('Generating ILTBI TEST data: ' + str(len(self.test_tb['images'].tolist())) + ' imgs')
                self.AB_paths = [opt.dataroot + img_path for img_path in self.test_tb['images'].tolist()]
        ####
