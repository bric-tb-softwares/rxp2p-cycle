...
class CycleManausSkfold2GeneratorDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.generate_paths_data_csv:
            ...
            print('Downloading....')
            df_iltbi = dorothy_dataset(opt.token, 'imageamento_anonimizado_valid', True, opt.dataset_download_dir)
            df_iltbi = df_iltbi.sort_values('project_id')
            df_manaus = dorothy_dataset(opt.token, 'manaus', True, opt.dataset_download_dir)
        ...
        particao_manaus_file = open(path_manaus, "rb")
        particao_manaus = pickle.load(particao_manaus_file)
        particao_manaus_file.close()
        splits = particao_manaus[opt.test]
        ...
        if (opt.isTB == True):
            self.train_tb = training_data.loc[df_manaus.target == True]
            self.val_tb = validation_data.loc[df_manaus.target == True]
            self.test_tb = test_data.loc[df_manaus.target == True]
            ...
            self.train_ntb = training_data.loc[df_manaus.target == True]
            self.val_ntb = validation_data.loc[df_manaus.target == True]
            self.test_ntb = test_data.loc[df_manaus.target == True]
            if opt.train_dataset:
